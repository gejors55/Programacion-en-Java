...
public abstract class JBoard extends JComponent {
	...
	private void initGUI() {
		...
		addKeyListener(new KeyListener() {
			...
			@Override
			public void keyPressed(KeyEvent e) {//Presionar que haga algo sin soltar
			}
		});
		addMouseListener(new MouseListener() {

			@Override
			public void mouseReleased(MouseEvent e) {//soltar boton raton
			}

			@Override
			public void mousePressed(MouseEvent e) {//presionar boton
			}

			@Override
			public void mouseExited(MouseEvent e) {//cuando sale de estar encima
			}

			@Override
			public void mouseEntered(MouseEvent e) {//raton sobre algo
	...
	private void drawCell(int row, int col, Graphics g) {
		...
		//String a = "" + getImagine(p);
		//java.net.URL imgURL = getClass().getClassLoader().getResource(a);
	//	if (imgURL != null) {
	//	icon =  new ImageIcon(imgURL);
		//final Image img = icon.getImage();
		//g.drawImage(img, x + _SEPARATOR+2, y + _SEPARATOR+2, _CELL_WIDTH - 2*_SEPARATOR-4, _CELL_HEIGHT - 2*_SEPARATOR-4, null);
				//}
		if (p != null) {
			Color c = getColor(p);
			Shape s = getShape(p);
...
//	protected abstract String getImagine(int piece);
